...
| --- | --- | --- | --- | --- | --- | --- |
| `/[locale]` | Home dashboard: greeting, due reviews, continue, daily lessons, news, gamification | `features/home` (`HomePage`) | **Partial / upgraded** | Real: flashcard decks/cards + sync queue, daily lesson provider, device-local progress summary, route-backed API feature entry points. Backend runtime verification: live Keycloak/API still required for API-backed destination screens. | P0 | 2026-06-03 Home rebuild adds a mobile-native hero, today lesson, review/progress, loading/error/empty/partial states, light/dark/360dp tests, and real shortcuts to Learn, Review/Flashcards, Exam, Dictionary, Search, Kanji, Grammar, Saved, News, Magazine, Scenarios, Career, Rewards, Subscription. Web-only widgets (ads, push prompt, focus timer, companion, login bonus, weekly report, personalized feed) are omitted or routed honestly until mobile contracts are consumed; no fake metrics. |

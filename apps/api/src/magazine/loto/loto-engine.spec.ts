...
function draw(drawNumber: number): LotoDrawInput {
  ...
  return {
    ...
    mainNumbers: [base, base + 1, base + 2, base + 3, base + 4, base + 5].map(
      (n) => ((n - 1) % 43) + 1
    ),
...
describe("loto lab engine", () => {
  ...
  it("auto-detects game from Japanese headers", () => {
    const loto6Csv =
      "開催回,日付,第1数字,第2数字,第3数字,第4数字,第5数字,第6数字,BONUS数字,キャリーオーバー\n1,2024/1/1,1,5,10,20,30,40,7,0";
    const loto7Csv =
      "開催回,日付,第1数字,第2数字,第3数字,第4数字,第5数字,第6数字,第7数字,BONUS数字1,BONUS数字2,キャリーオーバー\n1,2024/1/1,1,5,10,15,20,25,30,2,7,0";
  ...
  it("generates deterministic 1-5 candidate sets", () => {
    ...
    expect(sets[0]?.bonusNumbers).toEqual([]);
  });
});

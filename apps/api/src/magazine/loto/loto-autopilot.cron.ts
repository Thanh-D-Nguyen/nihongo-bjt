 ...
 * import it, generate a probability-learning reference set for the next draw,
 * and publish a single top set. Until then each tick reports `waiting_result` and retries on the next
...
export class LotoAutopilotCron {
  ...
  private async tick(game: LotoGame) {
    ...
    try {
      ...
      if (result.status === "waiting_result") {
        ...
        this.logger.log(
          `[autopilot:${game}] published ${result.publishedSlug} (${result.publishedSet?.join(", ")})`
        );
      }
      // "already_done" → silent no-op
    } catch (e) {
  ...
  /** Recover automatically after downtime or a result published outside the evening window. */
  @Cron("15 6 * * *", { timeZone: "Asia/Tokyo" })
  async handleDailyCatchup() {
    await this.tick("loto6");
    await this.tick("loto7");
  }

  /** Also reconcile shortly after each API process starts. Idempotency lives in PostgreSQL. */
  @Timeout(30_000)
  async handleStartupCatchup() {
    await this.tick("loto6");
    await this.tick("loto7");
  }
}

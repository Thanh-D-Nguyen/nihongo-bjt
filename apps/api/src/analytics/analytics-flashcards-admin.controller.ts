...
export class AnalyticsFlashcardsAdminController {
  constructor(
    @Inject(AdminAuthService) private readonly auth: AdminAuthService,
    @Inject(AnalyticsFlashcardsAdminRepository) private readonly repo: AnalyticsFlashcardsAdminRepository
  ) {}

  @Get("summary")
  @ApiOperation({ summary: "Flashcards analytics KPI tiles + freshness." })
  ...
  @Get("timeseries")
  @ApiOperation({ summary: "Flashcards analytics chart series. Metrics: reviews | retention." })
  ...
  @Get("breakdown")
  @ApiOperation({ summary: "Flashcards analytics breakdown. Dimensions: by_rating (default) | by_deck." })
  ...
  @Post("export")
  @HttpCode(200)
  @ApiOperation({ summary: "Export flashcards analytics view as CSV (audited)." })
  async export(
    @Req() req: Request,
    ...
    if (parsed.data.view === "summary") {
      ...
      const s = await this.repo.breakdown({
        ...parsed.data,
        dimension: parsed.data.dimension ?? "by_rating",
        page: 1,
        pageSize: 1000
      });
  ...
  @Post("refresh")
  @HttpCode(202)
  @ApiOperation({ summary: "Request refresh of flashcards analytics rollups (rate-limited; audited)." })

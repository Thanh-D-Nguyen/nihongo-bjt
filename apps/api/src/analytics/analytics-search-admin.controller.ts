...
export class AnalyticsSearchAdminController {
  constructor(
    @Inject(AdminAuthService) private readonly auth: AdminAuthService,
    @Inject(AnalyticsSearchAdminRepository) private readonly repo: AnalyticsSearchAdminRepository
  ) {}

  @Get("summary")
  @ApiOperation({ summary: "Search analytics KPI tiles + freshness." })
  ...
  @Get("timeseries")
  @ApiOperation({ summary: "Search analytics chart series. Metrics: queries | zero_results | clicks." })
  ...
  @Get("breakdown")
  @ApiOperation({ summary: "Search analytics breakdown. Dimensions: top_queries (default) | zero_result_queries | by_source." })
  ...
  @Post("export")
  @HttpCode(200)
  @ApiOperation({ summary: "Export search analytics view as CSV (audited)." })
  async export(
    @Req() req: Request,
    ...
    if (parsed.data.view === "summary") {
      ...
      const s = await this.repo.timeseries({
        ...
        metric: parsed.data.metric ?? "queries"
      });
      ...
      const s = await this.repo.breakdown({
        ...parsed.data,
        dimension: parsed.data.dimension ?? "top_queries",
        page: 1,
        pageSize: 1000
      });
  ...
  @Post("refresh")
  @HttpCode(202)
  @ApiOperation({ summary: "Request refresh of search analytics rollups (rate-limited; audited)." })

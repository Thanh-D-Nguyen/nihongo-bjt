...
export class AnalyticsSystemAdminController {
  constructor(
    @Inject(AdminAuthService) private readonly auth: AdminAuthService,
    @Inject(AnalyticsSystemAdminRepository) private readonly repo: AnalyticsSystemAdminRepository
  ) {}

  @Get("summary")
  @ApiOperation({ summary: "System analytics KPI tiles + freshness + partial_schema_pending notices." })
  ...
  @Get("timeseries")
  @ApiOperation({ summary: "System analytics chart series. Metrics: raw_events | rollup_runs." })
  ...
  @Get("breakdown")
  @ApiOperation({ summary: "System analytics breakdown. Dimensions: recent_runs (default) | by_event_source | by_rollup_status." })
  ...
  @Post("export")
  @HttpCode(200)
  @ApiOperation({ summary: "Export system analytics view as CSV (audited)." })
  async export(
    @Req() req: Request,
    ...
    if (parsed.data.view === "summary") {
      ...
      const s = await this.repo.timeseries({
        ...
        metric: parsed.data.metric ?? "raw_events"
      });
      ...
      const s = await this.repo.breakdown({
        ...parsed.data,
        dimension: parsed.data.dimension ?? "recent_runs",
        page: 1,
        pageSize: 1000
      });
  ...
  @Post("refresh")
  @HttpCode(202)
  @ApiOperation({ summary: "Request refresh of system analytics rollups (rate-limited; audited)." })

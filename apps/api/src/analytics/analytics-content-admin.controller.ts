...
export class AnalyticsContentAdminController {
  constructor(
    @Inject(AdminAuthService) private readonly auth: AdminAuthService,
    @Inject(AnalyticsContentAdminRepository) private readonly repo: AnalyticsContentAdminRepository
  ) {}

  @Get("summary")
  @ApiOperation({ summary: "Content analytics KPI tiles." })
  ...
  @Get("timeseries")
  @ApiOperation({ summary: "Content analytics chart series. Metrics: card_links_created | lexeme_added." })
  ...
  @Get("breakdown")
  @ApiOperation({ summary: "Content analytics breakdown. Dimensions: top_engaged (default) | by_type | by_lexeme_status." })
  ...
  @Post("export")
  @HttpCode(200)
  @ApiOperation({ summary: "Export content analytics view as CSV (audited)." })
  async export(
    @Req() req: Request,
    ...
    if (parsed.data.view === "summary") {
      ...
      const s = await this.repo.timeseries({
        ...
        metric: parsed.data.metric ?? "card_links_created"
      });
      ...
      const s = await this.repo.breakdown({
        ...parsed.data,
        dimension: parsed.data.dimension ?? "top_engaged",
        page: 1,
        pageSize: 1000
      });
  ...
  @Post("refresh")
  @HttpCode(202)
  @ApiOperation({ summary: "Request refresh of content analytics rollups (rate-limited; audited)." })

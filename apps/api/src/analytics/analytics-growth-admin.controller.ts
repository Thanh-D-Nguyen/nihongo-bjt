...
export class AnalyticsGrowthAdminController {
  constructor(
    @Inject(AdminAuthService) private readonly auth: AdminAuthService,
    @Inject(AnalyticsGrowthAdminRepository) private readonly repo: AnalyticsGrowthAdminRepository
  ) {}

  @Get("summary")
  @ApiOperation({ summary: "Growth analytics KPI tiles + freshness." })
  ...
  @Get("timeseries")
  @ApiOperation({ summary: "Growth analytics chart series. Metrics: signups | share_events | referral_events." })
  ...
  @Get("breakdown")
  @ApiOperation({ summary: "Growth analytics breakdown. Dimensions: by_campaign (default) | by_referral_kind | by_share_kind." })
  ...
  @Post("export")
  @HttpCode(200)
  @ApiOperation({ summary: "Export growth analytics view as CSV (audited)." })
  async export(
    @Req() req: Request,
    ...
    if (parsed.data.view === "summary") {
      ...
      const s = await this.repo.timeseries({
        ...
        metric: parsed.data.metric ?? "signups"
      });
      ...
      const s = await this.repo.breakdown({
        ...parsed.data,
        dimension: parsed.data.dimension ?? "by_campaign",
        page: 1,
        pageSize: 1000
      });
  ...
  @Post("refresh")
  @HttpCode(202)
  @ApiOperation({ summary: "Request refresh of growth analytics rollups (rate-limited; audited)." })

...
export class AnalyticsBjtAdminController {
  constructor(
    @Inject(AdminAuthService) private readonly auth: AdminAuthService,
    @Inject(AnalyticsBjtAdminRepository) private readonly repo: AnalyticsBjtAdminRepository
  ) {}

  @Get("summary")
  @ApiOperation({ summary: "BJT analytics KPI tiles + freshness." })
  ...
  @Get("timeseries")
  @ApiOperation({ summary: "BJT analytics chart series. Metrics: attempts | pass_rate." })
  ...
  @Get("breakdown")
  @ApiOperation({ summary: "BJT analytics breakdown. Dimensions: by_section (default) | by_test." })
  ...
  @Post("export")
  @HttpCode(200)
  @ApiOperation({ summary: "Export BJT analytics view as CSV (audited)." })
  async export(
    @Req() req: Request,
    ...
    if (parsed.data.view === "summary") {
      ...
      const s = await this.repo.timeseries({
        ...
        metric: parsed.data.metric ?? "attempts"
      });
      ...
      const s = await this.repo.breakdown({
        ...parsed.data,
        dimension: parsed.data.dimension ?? "by_section",
        page: 1,
        pageSize: 1000
      });
  ...
  @Post("refresh")
  @HttpCode(202)
  @ApiOperation({ summary: "Request refresh of BJT analytics rollups (rate-limited; audited)." })

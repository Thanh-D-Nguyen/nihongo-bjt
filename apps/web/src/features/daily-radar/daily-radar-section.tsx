...
function cardHref(card: DailyRadarCard, locale: string) {
  if (card.targetRoute) return card.targetRoute.replace(/^\/vi\//, `/${locale}/`);
  if (card.module.moduleKey === "workplace_mission") return `/${locale}/daily-standup`;
  return `/${locale}/modules/${card.module.moduleKey}`;
}
...
function Meta({ card, labels }: { card: DailyRadarCard; labels: DailyRadarLabels }) {
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-slate-600">
      ...
      <span className="rounded-full bg-white/70 px-2 py-0.5 ring-1 ring-slate-200">{card.moduleType}</span>
    </div>
  );
}
    ...
    >
      ...
      <div className="relative grid min-h-[280px] gap-6 lg:grid-cols-[minmax(0,1fr)_15rem]">
        <div className="flex flex-col">
          ...
          {card.recommendationReasonVi ? (
            <p className="mt-4 max-w-2xl rounded-2xl bg-white/12 px-4 py-3 text-sm leading-relaxed text-white/85 ring-1 ring-white/15">
              {card.recommendationReasonVi}
            </p>
          ) : null}
          <div className="mt-auto flex flex-wrap items-center gap-3 pt-6">
            ...
            <span className="text-xs font-medium text-white/75">
              {[card.levelLabel, card.estimatedMinutes ? labels.minutes.replace("{n}", String(card.estimatedMinutes)) : null]
                .filter(Boolean)
                .join(" · ")}
            </span>
          </div>
  ...
  return (
    <section aria-busy aria-labelledby="daily-radar-heading" className="space-y-4">
      ...
      <div className="flex gap-3 overflow-hidden">
        {[0, 1, 2].map((i) => <div className="h-52 w-80 shrink-0 animate-pulse rounded-2xl bg-slate-100" key={i} />)}
      </div>
    </section>
  );
}
...
export function DailyRadarSection({ labels, locale }: { labels: DailyRadarLabels; locale: string }) {
  ...
  async function load() {
    ...
    setError(false);
    try {
      setPayload(await fetchDailyRadarHome());
    } catch {
      setPayload(null);
      setError(true);
    } finally {
  ...
  if (!payload || (!payload.spotlight && payload.cards.length === 0)) {
    return (
  ...
  return (
    <section aria-labelledby="daily-radar-heading" className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {categories.map((item) => (
          <button
            className={`shrink-0 rounded-full px-3 py-1.5 text-sm font-semibold ring-1 transition ${category === item ? "bg-slate-950 text-white ring-slate-950" : "bg-white text-slate-700 ring-slate-200 hover:bg-slate-50"}`}
            key={item}
      ...
      {cards.length === 0 ? (
        ...
        <div ref={railRef} className="-mx-4 flex snap-x gap-3 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0">
          {cards.map((card) => <RadarCard card={card} key={card.id} labels={labels} locale={locale} />)}
        </div>
      )}
    </section>
  );

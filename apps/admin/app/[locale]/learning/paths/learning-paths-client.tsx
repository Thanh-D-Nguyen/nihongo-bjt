...
import { LEARNING_PATH_STATUSES } from "@nihongo-bjt/shared";
...
export function LearningPathsAdminClient({
  common,
  labels
}: {
  common: CommonLabels;
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (targetLevel !== "all") params.set("targetLevel", targetLevel);
      params.set("page", String(page));
      params.set("pageSize", String(PAGE_SIZE));
      const r = await adminApiFetch(`/api/admin/learning/paths?${params.toString()}`);
      if (!r.ok) {
        setData(null);
  ...
  const openDetail = useCallback(async (id: string) => {
    ...
    setDetail(null);
    try {
      const r = await adminApiFetch(`/api/admin/learning/paths/${id}`);
      if (r.ok) {
        setDetail((await r.json()) as LearningPathDetail);
      }
    } finally {
  ...
  const submitForm = async () => {
    ...
    const errs = validateFields([
      { field: "slug", value: form.slug, message: t("errorSlugRequired"), validate: validators.required },
    ...
    try {
      ...
      const url = editing === "create"
        ...
        : `/api/admin/learning/paths/${detail?.id}`;
      ...
      const saved = (await r.json()) as LearningPathDetail;
      setDetail(saved);
      setEditing(null);
  ...
  const submitTransition = async () => {
    ...
    try {
      ...
      const url = action === "delete"
        ? `/api/admin/learning/paths/${targetId}`
        : `/api/admin/learning/paths/${targetId}/${action}`;
    ...
    <div className="space-y-6">
      ...
      <AdminSection description={t("filterDescription")} title={t("filterTitle")}>
        <div className="flex flex-wrap items-end gap-3">
          ...
          <label className="flex flex-col text-xs">
            <span className="mb-1 font-medium text-slate-600">{t("filterTargetLevel")}</span>
            <select
              className="rounded-md border border-slate-300 px-2 py-1.5 text-sm"
              onChange={(e) => {
                setPage(1);
            ...
            >
              <option value="all">{t("status_all")}</option>
              {["BJT-J5", "BJT-J4", "BJT-J3", "BJT-J2", "BJT-J1"].map((l) => (
            ...
            <AdminDataTable>
              <AdminDataTableHead>
                <AdminDataTableRow>
                  ...
                  <AdminDataTableTh>{t("colTargetLevel")}</AdminDataTableTh>
                  <AdminDataTableTh>{t("colDisplayOrder")}</AdminDataTableTh>
              ...
              <AdminDataTableBody>
                {items.map((row) => (
                  ...
                  >
                    <AdminDataTableTd>
                      <span className="font-mono text-xs text-slate-700">{row.slug}</span>
                    </AdminDataTableTd>
                    <AdminDataTableTd>
                      <span className="font-medium text-slate-900">{row.titleVi}</span>
                    </AdminDataTableTd>
                    <AdminDataTableTd>{row.targetLevel ?? "—"}</AdminDataTableTd>
                    <AdminDataTableTd>{row.displayOrder}</AdminDataTableTd>
                    <AdminDataTableTd>
              ...
              {editing && !drawerLoading ? (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <FormField label={<>{t("formSlug")} <span className="text-red-500">*</span></>}>
                      <input
                        className={cn("w-full rounded-md px-2 py-1.5 text-sm", fe.fieldError("slug") ? "border border-red-400 bg-red-50/50" : "border border-slate-300")}
                        onChange={(e) => { fe.clearFieldError("slug"); setForm({ ...form, slug: e.target.value }); }}
                        value={form.slug}
                      />
                      {fe.fieldError("slug") && <p className="text-xs text-red-600">{fe.fieldError("slug")}</p>}
                    </FormField>
                    <FormField label={t("formTargetLevel")}>
                      <select
                    ...
                    <FormField label={t("formDisplayOrder")}>
                      <input
                        className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
                  ...
                  <FormField label={t("formDescriptionJa")}>
                    <textarea
                      className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm"
              ...
              {!editing && detail && !drawerLoading ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-slate-500">{detail.slug}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <DetailField label={t("formTargetLevel")}>{detail.targetLevel ?? "—"}</DetailField>
                    <DetailField label={t("formDisplayOrder")}>{detail.displayOrder}</DetailField>
                    <DetailField label={t("formTitleVi")}>{detail.titleVi}</DetailField>
                  ...
                  {canWrite ? (
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        className="rounded-md border border-slate-300 bg-slate-50 px-3 py-1.5 text-xs font-medium hover:bg-slate-100"
                        onClick={() => setReasonModal({ action: "duplicate", targetId: detail.id })}
                        type="button"
                      >
                        {t("actionDuplicate")}
                      </button>

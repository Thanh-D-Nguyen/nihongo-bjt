...
import { downloadCsv, permsFromMe, type MePayload } from "@/app/_components/admin-client-utils";
...
type Detail = Deck & { descriptionVi: string | null; descriptionJa: string | null; audit: AuditEntry[] };
...
export function DeckAdminClient({ labels }: { labels: Labels }) {
  ...
  useEffect(() => { setPage(1); }, [debounced, statusFilter, visibilityFilter]);
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      if (!r.ok) { setListError(t("error")); setList(null); return; }
      setListError(null);
      setList((await r.json()) as ListResponse);
    } catch { setListError(t("error")); }
  }, [debounced, statusFilter, visibilityFilter, page]);
  ...
  function closeDrawer() { setSelected(null); setDetail(null); setTransitioning(null); }
  ...
  async function submitTransition(next: "active" | "archived" | "draft") {
    ...
    try {
      ...
      if (!r.ok) { setToast({ kind: "err", text: t("transitionFailed") }); return; }
      setDetail((await r.json()) as Detail);
      setTransitioning(null);
      setReason("");
  ...
  function exportCsv() {
    ...
    const rows = list.items.map((it) => [
      it.id, it.titleVi, it.titleJa ?? "", it.visibility, it.status,
      String(it._count?.cards ?? 0), it.ownerUserId ?? "", it.updatedAt,
    ]);
    downloadCsv(`decks-${Date.now()}.csv`, header, rows);
  }
    ...
    <div className="space-y-4">
      ...
      <AdminSection>
        {list == null ? (
          ...
          <AdminEmptyState title="Chưa có deck nào." />
        ) : (
          <AdminDataTable>
            <AdminDataTableHead>
              <AdminDataTableRow>
                <AdminDataTableTh>{t("colTitleVi")}</AdminDataTableTh>
            ...
            <AdminDataTableBody>
              {list.items.map((it) => (
                ...
                >
                  <AdminDataTableTd><span className="text-sm">{it.titleVi}</span></AdminDataTableTd>
                  <AdminDataTableTd><span className="text-sm text-gray-700">{it.titleJa ?? "—"}</span></AdminDataTableTd>
                  <AdminDataTableTd>
                  ...
                  <AdminDataTableTd><span className="text-sm">{it._count?.cards ?? 0}</span></AdminDataTableTd>
                  <AdminDataTableTd>
                    <span className="text-xs text-gray-500">{new Date(it.updatedAt).toLocaleString()}</span>
                  </AdminDataTableTd>
                </AdminDataTableRow>
              ))}
            </AdminDataTableBody>
        ...
        <div aria-modal className="fixed inset-0 z-50 flex justify-end bg-black/40" onClick={closeDrawer} role="dialog">
          <div className="h-full w-full max-w-2xl overflow-y-auto bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between">
              <div>
                <div className="text-xs uppercase tracking-wide text-gray-500">{selected.visibility} · {selected.status}</div>
                ...
                <div className="mt-1 text-xs text-gray-500">{t("colCards")}: {selected._count?.cards ?? 0}</div>
              </div>
              ...
              <>
                ...
                {canWrite ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {detail.status !== "active" ? (
                      <button className="rounded bg-emerald-600 px-3 py-1 text-sm text-white" onClick={() => { setTransitioning("active"); setReason(""); }} type="button">{t("approve")}</button>
                    ) : null}
                    {detail.status !== "archived" ? (
                      <button className="rounded bg-red-600 px-3 py-1 text-sm text-white" onClick={() => { setTransitioning("archived"); setReason(""); }} type="button">{t("reject")}</button>
                    ) : null}
                    {detail.status !== "draft" ? (
                      <button className="rounded border px-3 py-1 text-sm" onClick={() => { setTransitioning("draft"); setReason(""); }} type="button">{t("draft")}</button>
                    ) : null}
                  </div>
                ) : null}
                ...
                {transitioning ? (
                  <div className="mt-4 space-y-2 rounded border p-3">
                    <div className="text-sm font-semibold">{t(`confirm_${transitioning}`)}</div>
                    ...
                    <div className="flex justify-end gap-2">
                      <button className="rounded border px-3 py-1 text-sm" onClick={() => setTransitioning(null)} type="button">{t("cancel")}</button>
                      <button className="rounded bg-indigo-600 px-3 py-1 text-sm text-white disabled:opacity-50" disabled={mutating} onClick={() => void submitTransition(transitioning)} type="button">{t("confirm")}</button>
                    </div>
                  </div>
                ) : null}

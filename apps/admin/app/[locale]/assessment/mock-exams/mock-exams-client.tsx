...
const EXAM_TYPE_LABEL: Record<string, string> = {
  mock: "Mock (Luyện tập)",
  official: "Official (Đề thi chính thức)",
...
export function MockExamsAdminClient({
  ...
  useEffect(() => {
    setPage(1);
  }, [debounced, statusFilter, typeFilter, levelFilter]);
  ...
  }, [debounced, statusFilter, typeFilter, levelFilter, page, common.error]);
  ...
  function exportCsv() {
    ...
    const rows = list.items.map((it) => [
      ...
      it.status,
      it.level ?? "",
      String(it.timeLimitSeconds ?? ""),
      String(it._count.sections),
      String(it._count.sessions),
      it.createdAt,
      it.updatedAt
    ]);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-6 p-6">
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <label className="flex flex-col text-xs">
            ...
            >
              ...
              {ASSESSMENT_MOCK_EXAM_STATUSES.map((s) => (
          ...
          <label className="flex flex-col text-xs">
            <span className="mb-1 font-medium text-slate-600">{t("filterType") ?? "Loại đề"}</span>
            <select
              className="rounded border border-slate-300 px-3 py-2 text-sm"
            ...
            >
              <option value="all">{t("filterAll")}</option>
              {ASSESSMENT_EXAM_TYPES.map((et) => (
                <option key={et} value={et}>
                  {EXAM_TYPE_LABEL[et] ?? et}
                </option>
              ))}
            </select>
          </label>
            ...
            <AdminDataTable>
              <AdminDataTableHead>
                <AdminDataTableRow>
                  ...
                  <AdminDataTableTh>{t("colType") ?? "Loại"}</AdminDataTableTh>
                ...
                {list.items.map((it) => (
                  ...
                  >
                    <AdminDataTableTd>
                      ...
                      {it.titleJa ? (
                        <div className="text-xs text-slate-500">{it.titleJa}</div>
                      ) : null}
                    </AdminDataTableTd>
                    ...
                    <AdminDataTableTd>
                      ...
                      >
                        {EXAM_TYPE_LABEL[it.type] ?? it.type}
                      </span>
                    </AdminDataTableTd>
                    <AdminDataTableTd>{it.level ?? "—"}</AdminDataTableTd>
                    <AdminDataTableTd>
                      <AdminStatusBadge tone={STATUS_TONE[it.status] ?? "neutral"}>
                        {t(`status_${it.status}`)}
                      </AdminStatusBadge>
                    </AdminDataTableTd>
                    <AdminDataTableTd>
                      {it.timeLimitSeconds ? `${Math.round(it.timeLimitSeconds / 60)} min` : "—"}
                    </AdminDataTableTd>
                    <AdminDataTableTd>{it._count.sections}</AdminDataTableTd>
                    <AdminDataTableTd>{it._count.sessions}</AdminDataTableTd>
                    <AdminDataTableTd className="text-xs text-slate-500">
                      {fmtWhen(it.updatedAt, locale)}
                    </AdminDataTableTd>
                  </AdminDataTableRow>
                ))}
              </AdminDataTableBody>
      ...
      {detail ? (
        ...
        >
          <div
            className="flex w-full max-w-3xl flex-col overflow-y-auto bg-white shadow-2xl"
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
              <div>
                ...
                <h2 className="text-lg font-semibold">
                  {detail.titleVi ?? detail.titleJa ?? detail.slug}
                </h2>
              </div>
              <button
            ...
            <div className="flex flex-col gap-4 p-6">
              <div className="flex flex-wrap items-center gap-3 text-sm">
                ...
                <span className="text-slate-600">
                  {detail.timeLimitSeconds
                    ? `${Math.round(detail.timeLimitSeconds / 60)} min`
                    : "—"}
                </span>
                <span className="text-slate-600">
                  · {t("audienceEstimate")}: {detail.audienceEstimate}
                </span>
              </div>
              {canManage && detail.type !== "practice" ? (
              ...
              {detail.type === "practice" ? (
                <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">
                  {labels.practiceManagedInTemplates ??
                    "Đề practice đang hiển thị cho learner; chỉnh nội dung và lifecycle ở màn Quiz Templates."}
                </div>
              ) : null}
              ...
              <div>
                ...
                <ul className="rounded border border-slate-200">
                  ...
                  {detail.sections.length === 0 ? (
                    <li className="px-3 py-2 text-sm text-slate-500">{common.empty}</li>
                  ) : null}
                </ul>
              </div>
                  ...
                  {detail.audit.map((a) => (
                    <li className="rounded border border-slate-200 p-3 text-xs" key={a.id}>
                      ...
                      <div className="mt-1 text-slate-600">
                        {a.actor?.displayName ?? a.actor?.email ?? "—"} · {a.reason ?? "—"}
                      </div>
                    </li>
                  ))}
                  {detail.audit.length === 0 ? (
                    <li className="text-xs text-slate-500">{common.empty}</li>
                  ) : null}
                </ul>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-2xl rounded-lg bg-white p-6 shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs font-medium text-slate-600">{t("formLevel")}</span>
                <select
                  className="rounded border border-slate-300 px-3 py-2"
              ...
              <label className="flex flex-col gap-1">
                <span className="text-xs font-medium text-slate-600">{t("formTimeLimit")} (s)</span>
                <input
                  type="number"
                  min={300}
                  max={36000}
              ...
              <label className="col-span-2 flex flex-col gap-1">
                <span className="text-xs font-medium text-slate-600">{t("formDescription")}</span>
                <textarea
                  className="rounded border border-slate-300 px-3 py-2"
                    ...
                    <div className="grid grid-cols-12 gap-2" key={idx}>
                      <input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            sections: form.sections.map((x, i) =>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            sections: form.sections.map((x, i) =>
                              i === idx ? { ...x, durationMin: Number(e.target.value) } : x
                            )
                          })
                        }
                      />
                      <input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            sections: form.sections.map((x, i) =>
                              i === idx ? { ...x, questionCount: Number(e.target.value) } : x
                            )
                          })
                        }
                      />
                      <button
                        type="button"
                        className="col-span-1 rounded border border-red-300 text-xs text-red-600"
                        onClick={() =>
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                ...
                type="button"
              >
                {showForm === "create" ? t("createSubmit") : t("editSubmit")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-2xl">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              <button
                className={cn(
                  "rounded px-3 py-2 text-sm font-medium text-white disabled:opacity-50",
                  confirm === "delete" ? "bg-red-600" : "bg-slate-900"
                )}
                disabled={mutating}
                onClick={submitConfirm}
                type="button"
              >
                {t(`confirmSubmit_${confirm}`)}
              </button>
            </div>
          </div>
        </div>

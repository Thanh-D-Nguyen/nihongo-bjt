...
export type FlashcardThemeConfig = z.infer<typeof flashcardThemeConfigSchema>;

export type FlashcardThemeTier = "free" | "premium" | "exclusive";

export interface FlashcardThemeDefinition {
...
] as const satisfies readonly FlashcardThemeDefinition[];

export const DEFAULT_FLASHCARD_THEME = FLASHCARD_THEME_DEFINITIONS[0];

export function resolveFlashcardThemeDefinition(
  slug: string | null | undefined
): FlashcardThemeDefinition {
  return (
    FLASHCARD_THEME_DEFINITIONS.find((theme) => theme.slug === slug) ?? DEFAULT_FLASHCARD_THEME
  );
}

function hexToRgb(hex: string): [number, number, number] {
  if (!HEX_COLOR.test(hex)) throw new Error(`Unsupported color: ${hex}`);
  return [
...
export interface FlashcardThemeContrastResult {
  background: string;
  foreground: string;
  minimum: number;
  pair: string;
...
export interface FlashcardThemeValidationResult {
  contrast: FlashcardThemeContrastResult[];
  errors: string[];
  success: boolean;
}
...
const CONTRAST_PAIRS = [
  ...
  readonly [string, keyof FlashcardThemeConfig, keyof FlashcardThemeConfig, number]
>;

export function validateFlashcardTheme(
  themeId: string,
  input: unknown
): FlashcardThemeValidationResult {
  const parsed = flashcardThemeConfigSchema.safeParse(input);
  if (!parsed.success) {
    return {
      contrast: [],
      errors: parsed.error.issues.map(
        (issue) => `${themeId}.${issue.path.join(".") || "config"}: ${issue.message}`
      ),
      success: false
    };
  }
  ...
  const errors = contrast
    ...
    .map(
      (result) =>
        `${themeId}: ${result.pair} ${result.foreground}/${result.background} is ${result.ratio.toFixed(2)}:1; requires ${result.minimum.toFixed(1)}:1`
    );

  return { contrast, errors, success: errors.length === 0 };
}

export function parseFlashcardThemeConfig(input: unknown): FlashcardThemeConfig {
  return flashcardThemeConfigSchema.parse(input);
}

export function safeFlashcardThemeConfig(input: unknown): FlashcardThemeConfig {
  const parsed = flashcardThemeConfigSchema.safeParse(input);
  if (!parsed.success) return DEFAULT_FLASHCARD_THEME.config;
  return parsed.data;
}

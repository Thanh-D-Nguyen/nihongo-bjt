...
export const ADMIN_SYSTEM_ROLE_PERMISSION_MATRIX = {
    superadmin: ["*"],
    admin: [
        "admin.content.read",
        "admin.content.write",
 ...
 */
export function canListLearnerProfiles(codes) {
    const s = codes instanceof Set ? codes : new Set(codes);
    return (s.has(ADMIN_PERMISSION.supportUserRead) ||
 ...
 */
export function canCreateLearnerProfile(codes) {
    const s = codes instanceof Set ? codes : new Set(codes);
    return (s.has(ADMIN_PERMISSION.supportUserWrite) ||
        s.has(ADMIN_PERMISSION.supportUserLegacy) ||
 ...
 */
export function canInviteOrCreateUser(codes) {
    const s = codes instanceof Set ? codes : new Set(codes);
    return (s.has(ADMIN_PERMISSION.adminUsersCreate) ||
        s.has(ADMIN_PERMISSION.userCreate) ||
        s.has(ADMIN_PERMISSION.supportUserWrite) ||
        s.has(ADMIN_PERMISSION.supportUserLegacy));
}
 ...
 */
export function canReadSensitiveUserProfile(codes) {
    const s = codes instanceof Set ? codes : new Set(codes);
    return (s.has("*") ||
...
export function toRolePermissionSet(role) {

...
const emptyQueryToUndef = (v) => v === null || v === "" || (typeof v === "string" && v.trim() === "") ? undefined : v;
const optionalQueryInt = (min, max) => z.preprocess((v) => {
    if (v == null || v === "")
        return undefined;
    const n = Number(v);
...
export const adminContentSummaryQuerySchema = z
    ...
    .refine((d) => d.strokeCountMin == null || d.strokeCountMax == null || d.strokeCountMin <= d.strokeCountMax, { path: ["strokeCountMax"] });
...
export const adminPatchLexemeExampleBodySchema = z
    ...
    const { reason, ...fields } = b;
    void reason;
    return ["japaneseText", "reading", "translationVi", "status"].some((k) => fields[k] !== undefined);
}, { path: ["body"], message: "At least one field to update" });
...
export const BATTLE_CONFIG_BOT_DIFFICULTIES = ["easy", "medium", "hard"];
...
export const BATTLE_GAME_TYPE_LABELS = {
...
export const GROWTH_CAMPAIGN_CHANNELS = ["email", "push", "in_app"];
...
export const adminGrowthCampaignListQuerySchema = z.object({
    q: z
        ...
        .transform((s) => (s === "" ? undefined : s)),
    status: z
        .enum([...GROWTH_CAMPAIGN_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    channel: z
        .enum([...GROWTH_CAMPAIGN_CHANNELS, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    level: z
        .string()
...
export const adminGrowthPostcardListQuerySchema = z.object({
    ...
    kind: z
        .enum([...GROWTH_POSTCARD_EVENT_KINDS, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    status: z
...
export const adminGrowthSocialTemplateListQuerySchema = z.object({
    q: z.string().trim().max(200).optional().transform((s) => (s === "" ? undefined : s)),
    kind: z
        .enum([...GROWTH_SOCIAL_TEMPLATE_KINDS, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    status: z
        .enum(["all", "draft", "published", "archived"])
 ...
 */
export const BATTLE_MATCH_STATUSES = ["in_progress", "completed", "abandoned"];
 ...
 */
export const BATTLE_LEADERBOARD_WINDOWS = ["all", "30d", "90d"];
 ...
 */
export const BATTLE_BOT_DIFFICULTIES = ["easy", "medium", "hard"];
export const BATTLE_BOT_STATUSES = ["active", "disabled", "archived"];
export const BATTLE_BOT_STYLE_TOKENS = ["calm", "focused", "sharp"];
 ...
 */
export const BATTLE_ABUSE_KINDS = ["cheating", "harassment", "inappropriate", "afk", "other"];
export const BATTLE_ABUSE_SEVERITIES = ["low", "medium", "high", "critical"];
export const BATTLE_ABUSE_STATUSES = ["open", "triaged", "resolved", "dismissed", "escalated"];
export const BATTLE_ABUSE_ACTIONS = ["warning", "temp_ban", "perm_ban", "dismissed"];
...
export { aggregateByCategoryGroup, getGrammarCategoryGroupFilterClauses, GRAMMAR_CATEGORY_GROUP_CHART_ORDER, GRAMMAR_CATEGORY_GROUP_SPECS, mapGrammarCategoryToGroupId } from "./grammar-category-taxonomy.js";
export { ADMIN_PERMISSION, ADMIN_ROUTE_GROUP_PERMISSIONS, ADMIN_SYSTEM_ROLE, ADMIN_SYSTEM_ROLE_PERMISSION_MATRIX, canCreateLearnerProfile, canInviteOrCreateUser, canListLearnerProfiles, canReadSensitiveUserProfile, toRolePermissionSet } from "./admin-permissions.js";
/**
...
export const adminContentEnrichmentListQuerySchema = z.object({
    q: z
        ...
        .transform((s) => (s === "" ? undefined : s)),
    status: z
        .enum([...CONTENT_ENRICHMENT_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    type: z
        .enum([...CONTENT_ENRICHMENT_TYPES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    entityType: z
        .string()
...
export const adminContentVersionListQuerySchema = z.object({
    ...
    status: z
        .enum([...CONTENT_VERSION_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    from: optionalIsoDate,
    to: optionalIsoDate,
...
export const ASSESSMENT_EXAM_TYPES = ["mock", "official"];
export const ASSESSMENT_MOCK_EXAM_STATUSES = ["draft", "published", "archived"];
...
export const ASSESSMENT_QUESTION_STATUSES = ["draft", "published", "archived"];
...
export const ASSESSMENT_BJT_SECTION_LABELS = {
...
export const ASSESSMENT_BJT_SECTION_LABELS_JA = {
    LC_SCENE: "場面把握問題",
    LC_STATEMENT: "発言聴解問題",
...
export const adminMockExamListQuerySchema = z.object({
    q: z.string().trim().max(200).optional().transform((s) => (s === "" ? undefined : s)),
    status: z
        .enum([...ASSESSMENT_MOCK_EXAM_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    type: z
        .enum([...ASSESSMENT_EXAM_TYPES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    level: z
        .string()
...
export const adminQuizTemplateListQuerySchema = z.object({
    ...
    level: z
        ...
        .transform((s) => (s === "" || s === "all" ? undefined : s)),
    type: z
        .enum([...ASSESSMENT_QUIZ_TEMPLATE_TYPES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    page: z.coerce.number().int().min(1).default(1),
    pageSize: z.coerce.number().int().min(1).max(100).default(25)
...
const enforceSingleCorrectQuestionOptions = (options, ctx) => {
...
export const adminQuestionBankPatchSchema = z
    ...
    .superRefine((value, ctx) => {
    if (value.options)
        enforceSingleCorrectQuestionOptions(value.options, ctx);
});
export const adminQuestionBankListQuerySchema = z.object({
    q: z.string().trim().max(200).optional().transform((s) => (s === "" ? undefined : s)),
    status: z
        .enum([...ASSESSMENT_QUESTION_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    level: z
        .string()
    ...
    topic: z
        ...
        .transform((s) => (s === "" || s === "all" ? undefined : s)),
    difficulty: z
        .enum([...ASSESSMENT_QUESTION_DIFFICULTIES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    tags: z
...
export const adminQuizSessionListQuerySchema = z.object({
    q: z.string().trim().max(200).optional().transform((s) => (s === "" ? undefined : s)),
    status: z
        .enum([...ASSESSMENT_QUIZ_SESSION_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" ? undefined : s)),
    userId: optionalUuid,
    testId: optionalUuid,
...
export const adminRemediationRulePatchSchema = z
    ...
    .superRefine((value, ctx) => {
    if (value.thresholdFailedCount !== undefined &&
...
export const ANALYTICS_GRANULARITIES = ["day", "hour"];
const isoDateField = z
    ...
    .optional()
    .transform((s) => (s === "" ? undefined : s));
const optionalEnumLike = (max = 64) => z
    .string()
    .trim()
    .max(max)
    .optional()

 ...
 */
import { z } from "zod";
export const LEARNING_PATH_STATUSES = ["draft", "published", "archived"];
export const COMPETENCY_STATUSES = ["draft", "published", "archived"];
...
export const DAILY_CONTENT_LOCALES = ["vi", "ja"];
...
const optionalNullableText = (max) => z
    .string()
    .trim()
    .max(max)
    .nullable()
...
export const adminLearningPathListQuerySchema = z.object({
    q: optionalText(200),
    status: z
        .enum([...LEARNING_PATH_STATUSES, "all"])
        .optional()
...
export const adminDailyContentItemListQuerySchema = z.object({
    q: optionalText(200),
    status: z
        .enum([...DAILY_CONTENT_ITEM_STATUSES, "all"])
        .optional()
        .transform((s) => (s === "all" || s === undefined ? undefined : s)),
    locale: z.enum([...DAILY_CONTENT_LOCALES, "all"]).optional()
        .transform((s) => (s === "all" || s === undefined ? undefined : s)),

...
function normFlashText(value: string): string {
 ...
 */
export function isLikelyVietnameseLegalDisclaimerOnlyBack(persistedBackText: string): boolean {
 ...
 */
export function buildDailySuggestedFlashcardBack(bodyMd: string, explanationText: string): string {
  ...
  const learningExtra =
    !isMetaBody && expl.length > 0 && !hasLegalDisclaimer && expl !== body ? `\n\n${expl}` : "";
 ...
 */
export function repairDailyContentFlashcardBackIfNeeded(
  persistedBackText: string,
  ...
  const strictDisclaimerOnly =
    explTrim.length > 0 &&
    normFlashText(explanationText ?? "") === backN &&
    VI_LEGAL_SAFEGUARD_PATTERN.test(explTrim);
  const looseDisclaimerOnly =
    isLikelyVietnameseLegalDisclaimerOnlyBack(persistedBackText) &&
    body.length > 0 &&
    VI_LEGAL_SAFEGUARD_PATTERN.test(explTrim);

...
export interface QuizOption {
  text: string;
  isCorrect: boolean;
  /** Original metadata preserved through shuffle */
 ...
 */
export function shuffleQuizOptions(options: QuizOption[]): ShuffledQuizOption[] {
...
): { shuffledOptions: Array<{ key: string; text: string }>; correctKey: string } {
 ...
 */
export function generateBalancedPositions(totalQuestions: number, numOptions = 4): number[] {
...
export function validateAnswerDistribution(
  ...
  numOptions = 4,
  maxBiasPercent = 35,
): { valid: boolean; distribution: Record<string, number>; message?: string } {
  const total = correctPositions.length;
  if (total === 0) return { valid: true, distribution: {} };
  ...
  let biasedKey: string | undefined;
...
// ─────────────────────────────────────────────────────────────────────────────
// EXAM-LEVEL DISTRIBUTION — Session-level balanced shuffle with anti-patterns
// Matches ETS/JLPT/Pearson VUE standards
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Shuffle map format: { [questionId]: permutation }
 * permutation is the ordered array of original option keys in their new display positions.
 * e.g. ["C", "A", "D", "B"] means display slot A shows original C, slot B shows original A, etc.
 */
export type ExamShuffleMap = Record<string, string[]>;

interface QuestionOptionMeta {
  questionId: string;
  /** Original option keys in DB order, e.g. ["A", "B", "C", "D"] */
  optionKeys: string[];
 ...
 */
export function generateExamShuffleMap(questions: QuestionOptionMeta[]): ExamShuffleMap {
 ...
 */
function generateBalancedAntiConsecutive(total: number, numOptions: number): number[] {
 ...
 */
function passesAntiConsecutive(positions: number[], maxRun: number): boolean {
  let run = 1;
 ...
 */
function greedyAntiConsecutive(total: number, numOptions: number): number[] {
  ...
  const budget = Array.from({ length: numOptions }, (_, i) =>
  ...
  for (let i = 0; i < total; i++) {
    ...
    for (let p = 0; p < numOptions; p++) {
      ...
      if (lastTwo.length === 2 && lastTwo[0] === p && lastTwo[1] === p) continue;
      candidates.push(p);
    }
 ...
 */
function buildPermutation(optionKeys: string[], correctKey: string, targetPosition: number): string[] {
  const incorrect = optionKeys.filter((k) => k !== correctKey);
  const shuffledIncorrect = shuffleArray(incorrect);

  const result: string[] = new Array(optionKeys.length);
 ...
 */
export function applyShufflePermutation<T extends { optionKey: string }>(
  originalOptions: T[],
  permutation: string[],
 ...
 */
export function reverseMapDisplayKey(permutation: string[], displayKey: string): string {
 ...
 */
function seededShuffleArray<T>(arr: T[], rng: () => number): T[] {
  const result = [...arr];
  for (let i = result.length - 1; i > 0; i--) {
 ...
 */
function seededGreedyAntiConsecutive(total: number, numOptions: number, rng: () => number): number[] {
  const baseCount = Math.floor(total / numOptions);
  const remainder = total % numOptions;
  const budget = Array.from({ length: numOptions }, (_, i) =>
    i < remainder ? baseCount + 1 : baseCount
 ...
 */
function seededBuildPermutation(optionKeys: string[], correctKey: string, targetPosition: number, rng: () => number): string[] {
  const incorrect = optionKeys.filter((k) => k !== correctKey);
 ...
 * @returns ExamShuffleMap keyed by questionId
 */
export function generateSeededExamShuffleMap(questions: QuestionOptionMeta[], seed: string): ExamShuffleMap {
  const total = questions.length;
  if (total === 0) return {};

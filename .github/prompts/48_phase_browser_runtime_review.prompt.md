...
5. For a full-admin visual audit, set `BROWSER_REVIEW_APP=admin`, `BROWSER_REVIEW_LOCALE=vi` unless another locale is specified, and `BROWSER_REVIEW_ROUTES=__ADMIN_ALL__` so the runner expands every visible admin nav `href`. If 81 routes exceed the timeout, split into bounded batches and record each report path.
...
11. If browser cannot run, record `blocked_environment` with exact command/error and report path, then continue to Release Director.
12. Do not mark pass without browser/runtime evidence unless the status is `blocked_environment` with a bounded-runner report.
13. Never wait indefinitely; after runner timeout, continue with `blocked_environment` evidence.
14. Do not retry the same browser command more than once unless a concrete UI/browser blocker was fixed.
15. Browser visual audit pending is not a human approval boundary. Return evidence and `next_action: release gate | fix blocker`; do not ask for human review unless the next action is final public launch/go-live.
</instructions>
